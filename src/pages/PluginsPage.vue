          ...
          <div v-else>
            ...
            <div v-else class="row q-col-gutter-md">
              <div
                ...
                :key="plugin.id"
                v-memo="[plugin.id, plugin.installed, plugin.disabled]"
                ...
                <q-card class="plugin-card">
                  <q-item>
                    ...
                    <q-item-section side>
                      ...
                      <q-badge
                        :color="plugin.installed ? 'positive' : 'grey'"
                        outline
                      >
              ...
              <!-- 加载更多按钮 -->
              <div
                v-if="visiblePlugins.length < filteredPlugins.length"
                class="col-12 flex justify-center q-mt-md"
              >
                <q-btn
                  color="primary"
                  label="加载更多"
    ...
    <q-dialog v-model="logsDialogVisible" maximized persistent>
      <q-card>
        ...
        <q-card-actions align="right">
          ...
          <q-btn
            v-if="
              selectedOperation?.type === 'install' &&
...
const filterPlugins = () => {
  ...
  const filtered = plugins.value.filter((plugin) => {
    ...
    if (tagFilter.value.length > 0) {
      matchesTags = tagFilter.value.some((tag) => plugin.tags?.includes(tag));
    }
...
const filteredPlugins = computed(() => {
  ...
  return plugins.value.filter((plugin) => {
    // 先检查最快的过滤器（如状态过滤）
    if (statusFilter.value.value === 'installed' && !plugin.installed)
      return false;
    if (statusFilter.value.value === 'not-installed' && plugin.installed)
      return false;

    // 然后检查标签过滤（如果有）
    if (
      tagFilter.value.length > 0 &&
      !plugin.tags?.some((tag) => tagFilter.value.includes(tag))
    ) {
      return false;
    }
...
// 代理选项
const proxyOptions = ref<string[]>(['', 'https://gh-proxy.com/']);
      ...
      try {
        ...
        if (completed) {
          ...
          if (progress === 100) {
            ...
            const errorMsg =
              message ||
              `${type === 'install' ? '安装' : '卸载'} ${pluginId} 失败`;
        ...
        $q.notify({
          color: 'negative',
          message: `进度请求失败: ${
            error instanceof Error ? error.message : '连接错误'
          }`,
          icon: 'error',
          position: 'top',
          timeout: 5000,
...
const togglePluginState = async (plugin: Plugin) => {
  try {
    ...
    // 显示进度对话框
    overallProgress.value = 0;
    installationMessage.value = `正在${plugin.disabled ? '启用' : '禁用'} ${
      plugin.name
    }...`;
    progressVisible.value = true;
...
// 添加轮询插件状态变更进度的函数
const pollPluginStateChange = async (
  taskId: string,
  pluginId: string,
      ...
      try {
        ...
        if (completed) {
          ...
          if (progress === 100) {
            ...
            const errorMsg =
              message ||
              `${action === 'enable' ? '启用' : '禁用'} ${pluginId} 失败`;
            console.error(`操作失败: ${errorMsg}`);
