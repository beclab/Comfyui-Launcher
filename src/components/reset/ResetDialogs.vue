<template>
  ...
  <q-dialog v-model="showResetDialog1" persistent>
    <q-card>
      ...
      <q-card-actions align="right" style="margin-bottom: 8px;margin-right: 8px;">
        ...
        <q-btn :label="t('reset.dialog1.confirmButton')" color="negative" @click="showResetConfirmDialog" style="padding-top: 8px; padding-bottom: 8px;"/>
      </q-card-actions>
    </q-card>
  </q-dialog>
  ...
  <q-dialog v-model="showResetProgress" persistent>
    <q-card style="min-width: 550px; max-width: 80vw;">
      <q-card-section>
        <div class="text-h6">{{ t('reset.progress.title') }}</div>
      </q-card-section>
  ...
  <q-dialog v-model="showResetComplete" persistent>
    <q-card>
      <q-card-section class="row items-center">
        ...
        <span class="q-ml-sm">{{ t('reset.complete.title') }}</span>
      </q-card-section>
      
      <q-card-section>
        <p>{{ t('reset.complete.message') }}</p>
...
export default {
  ...
  setup() {
    ...
    const resetComfyUI = async () => {
      ...
      resetLogs.value = [t('reset.progress.starting')]
      isResetting.value = true
      
      try {
        const response = await api.resetComfyUI(lang.value)
